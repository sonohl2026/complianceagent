...
KNOWN, DOCUMENTED LIMITATION (found running the real suite, not a bug):
Stage 2 (spec-scoped to openFDA + CMS Coverage API only) has NO source at
all for CPT/HCPCS codes or PFS/OPPS/DMEPOS payment rates -- openFDA never
returns billing codes, and CMS's Coverage Database only documents a
code/rate when a device happens to have its own dedicated NCD/LCD/Article,
which most 510(k)-riding-existing-codes devices don't (their payment flows
through an already-priced code with no separate coverage determination ever
issued -- confirmed by direct query against CMS's real Coverage Database and
its open-data catalog: no queryable HCPCS/CPT code-and-rate table exists
there, only bulk quarterly files). This means the coding/coverage/payment
pillars are structurally unassessable for many real devices, correctly
tripping the <3-assessed-pillars NOT_SCORED rule -- honest, spec-compliant
behavior, not a retrieval or prompt bug. `_classify` below distinguishes
this from a genuine regression: KNOWN_GAP (thin real evidence, no technical
failure) vs FAIL (anything else, including a RETRIEVAL_FAILURE-driven miss
or a wrong SCORED band). Closing this gap for real would mean ingesting
CMS's bulk HCPCS/PFS files -- an unplanned scope expansion, not attempted
this pass; see conversation record for the investigation.
"""
